...
use role accountadmin;
    ...
    CREATE OR REPLACE API INTEGRATION git_api_integration
        ...
        API_ALLOWED_PREFIXES = ('https://github.com/Manu-RMT/')
        ENABLED = TRUE;
...
use role SF_Intelligence_Demo;
    ...
    CREATE OR REPLACE GIT REPOSITORY SF_AI_DEMO_REPO
        ...
        ORIGIN = 'https://github.com/Manu-RMT/Snowflake_IA_Agent_Demo.git';
    ...
    FROM @SF_AI_DEMO_REPO/branches/master/demo_data/;


    COPY FILES
    INTO @INTERNAL_DATA_STAGE/unstructured_docs/
    FROM @SF_AI_DEMO_REPO/branches/master/unstructured_docs/;
  ...
  -- SELECT 'Internal Stage Files' as stage_type, COUNT(*) as file_count FROM (LS @INTERNAL_DATA_STAGE);
    ...
    DROP SCHEMA SF_AI_DEMO.DEMO_SCHEMA;
    
    CREATE SCHEMA SF_AI_DEMO.DEMO_SCHEMA
    CLONE SF_AI_DEMO.PUBLIC;

    SHOW TABLES IN SCHEMA SF_AI_DEMO.PUBLIC;

    SHOW TABLES IN SCHEMA SF_AI_DEMO.DEMO_SCHEMA;

    DROP SCHEMA SF_AI_DEMO.PUBLIC CASCADE;

    CREATE SCHEMA SF_AI_DEMO.PUBLIC;
...
create or replace semantic view SF_AI_DEMO.DEMO_SCHEMA.HR_SEMANTIC_VIEW
  ...
  SHOW SEMANTIC METRICS; 
    ...
    -- UNSTRUCTURED DATA
    -- ========================================================================
SHOW STAGES IN SCHEMA SF_AI_DEMO.DEMO_SCHEMA;

    
-- Création (ou remplacement) de la table qui contiendra
-- le contenu extrait des documents PDF présents dans le stage.
CREATE OR REPLACE TABLE parsed_content AS
SELECT

    -- Chemin du fichier dans le stage
    -- Exemple : unstructured_docs/guide_snowflake.pdf
    relative_path,

    -- Construction de l'URL Snowflake permettant d'accéder au fichier
    BUILD_STAGE_FILE_URL(
        '@SF_AI_DEMO.DEMO_SCHEMA.INTERNAL_DATA_STAGE',
        relative_path
    ) AS file_url,

    -- Conversion de l'URL en objet FILE Snowflake.
    -- Cet objet peut être réutilisé dans Cortex Search,
    -- Streamlit ou d'autres fonctionnalités IA.
    TO_FILE(
        BUILD_STAGE_FILE_URL(
            '@SF_AI_DEMO.DEMO_SCHEMA.INTERNAL_DATA_STAGE',
            relative_path
    ...
    -- Extraction du contenu du PDF avec Cortex PARSE_DOCUMENT.
    -- Le mode LAYOUT conserve autant que possible la structure
    -- du document (titres, paragraphes, tableaux, listes, colonnes).
    -- Le texte extrait est récupéré depuis la propriété "content"
    -- du JSON retourné puis converti en STRING.
...
-- Lecture de l'inventaire des fichiers présents dans le stage.
-- La fonction DIRECTORY retourne une ligne par fichier.
FROM DIRECTORY(@SF_AI_DEMO.DEMO_SCHEMA.INTERNAL_DATA_STAGE)

-- On ne conserve que les fichiers PDF présents dans le dossier
-- unstructured_docs du stage.
...
-- Network Rule utilisée pour autoriser les connexions réseau sortantes
-- depuis Snowflake vers des ressources externes (sites web, API, services cloud).
CREATE OR REPLACE NETWORK RULE Snowflake_intelligence_WebAccessRule
  -- EGRESS = trafic sortant de Snowflake vers l'extérieur
  MODE = EGRESS
  -- La règle est définie à partir d'un hôte et d'un port réseau
  TYPE = HOST_PORT
  -- Autorise les connexions HTTP (80) et HTTPS (443)
  -- vers n'importe quelle destination Internet.
  VALUE_LIST = (
    '0.0.0.0:80',
    '0.0.0.0:443'
  );
...
-- Donne tous les privilèges sur la base SF_AI_DEMO au rôle ACCOUNTADMIN.
-- Cela inclut notamment l'utilisation de la base et la gestion de ses objets.
GRANT ALL PRIVILEGES ON DATABASE SF_AI_DEMO TO ROLE ACCOUNTADMIN;
-- Donne tous les privilèges sur le schéma DEMO_SCHEMA.
-- ACCOUNTADMIN peut créer, modifier, supprimer et utiliser
-- les objets contenus dans ce schéma.
GRANT ALL PRIVILEGES ON SCHEMA SF_AI_DEMO.DEMO_SCHEMA TO ROLE ACCOUNTADMIN;
-- Autorise le rôle ACCOUNTADMIN à utiliser la Network Rule
-- snowflake_intelligence_webaccessrule.
-- Cette permission
USE SCHEMA SF_AI_DEMO.DEMO_SCHEMA;


-- Création d'une External Access Integration.
-- Cette intégration permet aux procédures Python, UDF ou applications
-- Snowflake autorisées d'effectuer des appels réseau externes.
CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION Snowflake_intelligence_ExternalAccess_Integration
-- Liste des règles réseau autorisées.
-- Ici, l'intégration utilise la Network Rule créée précédemment.
ALLOWED_NETWORK_RULES = (Snowflake_intelligence_WebAccessRule)
-- Active l'intégration.
ENABLED = TRUE;


-- Création d'une Notification Integration de type EMAIL.
-- Cette intégration permet à Snowflake d'envoyer des emails.
CREATE OR REPLACE NOTIFICATION INTEGRATION ai_email_int
-- Type d'intégration : courrier électronique.
TYPE = EMAIL
-- Active l'intégration.
ENABLED = TRUE;
